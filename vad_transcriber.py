...
class WhisperTranscriber:
    ...
    def _transcribe_sync(self, audio: np.ndarray) -> Optional[TranscriptSegment]:
        segments, info = self.model.transcribe(
            ...
            language=None,
            task="transcribe",
            beam_size=5,                             # Mantenemos la precisión
            vad_filter=True,                         # Mantenemos el filtro de ruido
            temperature=0.0,
            no_speech_threshold=0.5,
            condition_on_previous_text=True          # Mantenemos el contexto (para arreglar lo de "negocios")
            
            # ELIMINADO: initial_prompt. Hacía más daño que bien.
        )

         # 1. BLOQUEO ABSOLUTO: Si no es 'en' ni 'es', se destruye.
        if info.language not in ("en", "es"):
            return None

        # 2. TU NUEVA CONDICIÓN (Filtro de duda): 
        # Si dice que es inglés o español, pero está menos del 40% seguro, probablemente sea ruido.
